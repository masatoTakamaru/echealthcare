<x-app-layout>
<h1>商品の新規登録</h1>
<form action="{{ route('admin.item.create') }}" method="GET">
    @csrf
...
<form action="{{ route('admin.item.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <input type="text" id="maker" name="maker" size="40" value="{{ old('maker') }}">
    ...
    <tbody>
        @for($i = 1; $i <= 5; $i++)
            <tr class="border-2">
                ...
                <td class="p-2">
                    <input type="file" id="image{{ $i }}" name="image{{ $i }}" value="{{ old('image' . $i) }}">
    ...
    <input class="block mt-8 mb-20 bg-blue-200 py-2 px-8 rounded" type="submit" value="商品の登録">
</form>
</x-app-layout>
